...
def lookback_Put_CRR(StMax, St, T, r, q, sigma, layers, type, print_out = False):
    dt = T/layers
    u = exp(sigma * sqrt(dt))
    d = exp(-sigma * sqrt(dt))
    p = (exp((r-q)*dt) - d)/(u - d)
    ...
    putValue = TreeNodes[0][0].putValue[0]
    
    if print_out == True:
        print(f'(CRR Binomial Tree) Price of {type} Lookback Put : {round(putValue, 4)}')
...
# layers = 100
# print(f'n = {layers}')
# lookback_Put_CRR(StMax, St, T, r, q, sigma, layers, 'European')
# lookback_Put_CRR(StMax, St, T, r, q, sigma, layers, 'American')
# print('------------------------------------------------------------')
# layers = 300
# print(f'n = {layers}')
# lookback_Put_CRR(StMax, St, T, r, q, sigma, layers, 'European')
