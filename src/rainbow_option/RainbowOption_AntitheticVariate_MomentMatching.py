...
def rainbow_MC_AntitheticVariate(asset_amount, S0_lst, K, T, r, q_lst, sigma_lst, rho_dict, sims, reps, call_put, print_out = False):
    parameters = dict()
    ...
    while times < reps:
        ...
        for j in range(asset_amount):
            ...
            # 假如sims不為偶數
                ...
                print('-----------------------------------------------------')
                print('!! Exception : input of sims is odd --> sims = sims - 1 !!')
                print('-----------------------------------------------------')
                sims -= 1
                for i in range(sims):
                    temp.append(zzz[i][j])
    ...
    if print_out == True:
        print(f'(MC with variance-reduction approaches) Mean price of Rainbow European { temp_str} with {asset_amount} assets: {round(meanOfRep, 4)}')
